...
import { generateAccessTokenAndRefreshToken, decodeToken } from "../utils/authHelper";
...
export const userLogout = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ message: "An error occurred while logging out the user" });
  }
};
...
export const getCurrentUser = async (req: Request, res: Response) => {
  try {
   ...
   res.status(200).json({ user });
    ...
    res.status(500).json({ message: "An error occurred while getting the current user" });
    return
  }
};
